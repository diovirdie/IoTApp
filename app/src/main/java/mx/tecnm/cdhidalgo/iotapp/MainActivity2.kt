...
class MainActivity2 : AppCompatActivity(), ItemListener {
    ...
    private fun fill(){
        ...
        val peticion= object :JsonObjectRequest(Method.GET,url,null,{
            ...
            for (i in 0  until datos.length()){
                val sensors= datos.getJSONObject(i)
                lista[i][0] = sensors.getString("id")
                lista[i][1] = sensors.getString("name")
                lista[i][2] = sensors.getString("type")
                lista[i][3] = sensors.getString("value")
                lista[i][4] = sensors.getString("date")
            }
    ...
    override fun onClick(v: View?, position: Int) {
        Toast.makeText(this,"clic en posicion $position",Toast.LENGTH_SHORT).show()
    }
